__kernel void cnn_0_1_2_3_4_5( __global const float* b0, __global const float* b1, __global const float* b2, __global const float* b5, __global const float* b6, __global const float* b9, __global const float* b10, __global const float* b13, __global const float* b15, __global const float* b17, __global float* b3, __global float* b4, __global float* b7, __global float* b8, __global float* b11, __global float* b12, __global float* b14, __global float* b16, __global float* b18, __global float* b19)
{
	...
	// 				data1_C = (float3)(115.0f,212.0f,76.0f);
	// 				if(inputRow_C+i<0 || inputRow_C+i>=32)
	// 					data0_C.xyz = (float3)(0.0f,0.0f,0.0f);
	// 				else if(inputCol_C<0)
	// 				{
	// 					data0_C = vload3(0,(__global const float *)b0+(j*32*32)+(inputRow_C+i)*32+inputCol_C);
	// 					data0_C.x = 0;
	// 				}
	...
	// 					data0_C = vload3(0,(__global const float *)b0+(j*32*32)+(inputRow_C+i)*32+inputCol_C);
	// 				}
	// 				data1_C = vload3(0,(__global const float *)b1+(plane_C*3*3*3)+(j*3*3)+i*3);
	// 				dots += dot(data0_C,data1_C);
	// 			}
	// 		}
	// 		if(outputRow_C<32 && outputCol_C<32)
	// 			b3[0]=dots + b2[plane_C];
...
// 					else if(inputCol_C<0)
// 					{
// 						data0_C = vload3(0,(__global float *)b4+(j*16*16)+(inputRow_C+i)*16+inputCol_C);
// 						data0_C.x = 0;
// 					}
...
// 						data0_C = vload3(0,(__global float *)b4+(j*16*16)+(inputRow_C+i)*16+inputCol_C);
// 					}
// 					data1_C = vload3(0,(__global const float *)b5+(plane_C*3*3*16)+(j*3*3)+i*3);
// 					dots += dot(data0_C,data1_C);
// 				}
// 			}
// 			if(outputRow_C<16 && outputCol_C<16)
// 				b7[0] = dots + b6[plane_C];
...
// 					else if(inputCol_C<0)
// 					{
// 						data0_C = vload3(0,(__global float *)b8+(j*8*8)+(inputRow_C+i)*8+inputCol_C);
// 						data0_C.x = 0;
// 					}
...
// 						data0_C = vload3(0,(__global float *)b8+(j*8*8)+(inputRow_C+i)*8+inputCol_C);
// 					}
// 					data1_C = vload3(0,(__global const float *)b9+(plane_C*3*3*32)+(j*3*3)+i*3);
// 					dots += dot(data0_C,data1_C);
// 				}
// 			}
// 			if(outputRow_C<8 && outputCol_C<8)
// 				b11[0] = dots + b10[plane_C];
...
// 		// 	// wt= vload4(0,(__global const float *)b13+(globalId*1024));
// 		// 	// wt= vload4(0,(__global const float *)b13+(globalId*1024+4*x));
...
// // 			// wt= vload4(0,(__global const float *)b15+(globalId*512+4*0));
// //  			// dotProduct += dot(wt,temp);
// // /*			dotProduct += wt.x*temp.x + wt.y*temp.y + wt.z*temp.z + wt.w*temp.w;*/
// // 		}
...
// // 			// wt= vload4(0,(__global const float *)b17+(globalId*128+4*0));
// // 			// dotProduct += dot(wt,temp);
// // /*			dotProduct += wt.x*temp.x + wt.y*temp.y + wt.z*temp.z + wt.w*temp.w;*/
// // 		}
