__kernel void cnn_0_1_2_3_4_5_6_7_8_9( __global const float* b0, __global const float* b1, __global const float* b2, __global const float* b5, __global const float* b6, __global const float* b9, __global const float* b10, __global const float* b13, __global const float* b15, __global const float* b17, __global float* b3, __global float* b4, __global float* b7, __global float* b8, __global float* b11, __global float* b12, __global float* b14, __global float* b16, __global float* b18, __global float* b19)
{
				...
				for(int i=0;i<3;i++)
				{
					...
					else if(inputCol_C<0)
					{
						if(inputRow_C+i==0 && j==0)
						{
							...
							data0_C.xyz = vload3(0,(__global float *)b4+(j*16*16)+(inputRow_C+i)*16+inputCol_C);
						data0_C.x = 0;
					}
					else if(inputCol_C+3-1>=16)
					{
						...
						{
							data0_C.x = b4[(j*16*16)+(inputRow_C+i)*16+2];
							data0_C.y = b4[(j*16*16)+(inputRow_C+i)*16+3];
						}
						else
						data0_C.xyz = vload3(0,(__global float *)b4+(j*16*16)+(inputRow_C+i)*16+inputCol_C);
						data0_C.z = 0;
					...
					data0_C.w = 0.0;
					data1_C.xyz = vload3(0,(__global const float *)b5+(plane_C*3*3*16)+(j*3*3)+i*3);
					dots += dot(data0_C,data1_C);
				}
			}
			if(outputRow_C<16 && outputCol_C<16)
				b7[plane_C*16*16+outputRow_C*16+outputCol_C] = dots + b6[plane_C];
		}
	...
	if(globalId < 32*8*8)
	{
		...
		if(process)
		{
			...
			data1_P = vload2(0,(__global float *)b7+poolInputOffset+16);
				...
				for(int i=0;i<3;i++)
				{
					...
					else if(inputCol_C<0)
					{
						if(inputRow_C+i==0 && j==0)
						{
							...
							data0_C.xyz = vload3(0,(__global float *)b8+(j*8*8)+(inputRow_C+i)*8+inputCol_C);
						data0_C.x = 0;
					}
					else if(inputCol_C+3-1>=8)
					{
						...
						{
							data0_C.x = b8[(j*8*8)+(inputRow_C+i)*8+2];
							data0_C.y = b8[(j*8*8)+(inputRow_C+i)*8+3];
						}
						else
						data0_C.xyz = vload3(0,(__global float *)b8+(j*8*8)+(inputRow_C+i)*8+inputCol_C);
						data0_C.z = 0;
					...
					data0_C.w = 0.0;
					data1_C.xyz = vload3(0,(__global const float *)b9+(plane_C*3*3*32)+(j*3*3)+i*3);
					dots += dot(data0_C,data1_C);
				}
			}
			if(outputRow_C<8 && outputCol_C<8)
				b11[plane_C*8*8+outputRow_C*8+outputCol_C] = dots + b10[plane_C];
		}
	...
	if(globalId < 64*4*4)
	{
		...
		maxValue = b11[ poolInputOffset ];

		process = (inputRow_P + 1 < 8) && (inputCol_P + 1 < 8);

		if(process)
		{
			data0_P = vload2(0,(__global float *)b11+poolInputOffset);
			data1_P = vload2(0,(__global float *)b11+poolInputOffset+8);
	...
	if(globalId < 512)
	{
		for(int x=0; x<1024/4; x++)
		{
			...
			wt.x= b13[512*(4*x)+globalId];
			wt.y= b13[512*((4*x)+1)+globalId];
			wt.z= b13[512*((4*x)+2)+globalId];
			wt.w= b13[512*((4*x)+3)+globalId];
...
b14[globalId] = dotProduct;
	...
	if(globalId < 128)
	{
		for(int x=0; x<512/4; x++)
		{
			...
			wt.x= b15[128*(4*x)+globalId];
			wt.y= b15[128*((4*x)+1)+globalId];
			wt.z= b15[128*((4*x)+2)+globalId];
			wt.w= b15[128*((4*x)+3)+globalId];
			dotProduct += dot(wt,temp);
		}
b16[globalId] = dotProduct;
		printf("");
	}
	...
	if(globalId < 16)
	{
		...
		{
			temp= vload4(0,(__global float *)b16+(4*x));
			wt.x= b17[16*(4*x)+globalId];
			wt.y= b17[16*((4*x)+1)+globalId];
			wt.z= b17[16*((4*x)+2)+globalId];
			wt.w= b17[16*((4*x)+3)+globalId];
			dotProduct += dot(wt,temp);
		}
b18[globalId] = dotProduct;
		printf("");
	}
