__kernel void cnn_0_1_2( __global  float* b0, __global  float* b1, __global  float* b2, __global  float* b5, __global  float* b6, __global float* b3, __global float* b4, __global float* b7)
{
				...
				for(int i=0;i<3;i++)
				{
					...
					else if(inputCol_C<0)
					{
						if(inputRow_C+i==0 && j==0)
						{
							...
							data0_C.xyz = vload3(0,(__global  float *)b0+(j*16*16)+(inputRow_C+i)*16+inputCol_C);
						data0_C.x = 0;
					}
					...
					data0_C.w = 0.0;
					data1_C.xyz = vload3(0,(__global  float *)b1+(plane_C*3*3*3)+(j*3*3)+i*3);
		...
		inputCol_C = outputCol_C * stride - pad;
			...
			for(int j=0;j<16;j++)
			{
				for(int i=0;i<3;i++)
				{
					...
					data0_C.w = 0.0;
					data1_C.xyz = vload3(0,(__global  float *)b5+(plane_C*3*3*16)+(j*3*3)+i*3);
					dots += dot(data0_C,data1_C);
				}
			}
