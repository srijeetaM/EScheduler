__kernel void cnn_0_1_2_3_4_5_6( __global const float* b0, __global const float* b1, __global const float* b2, __global const float* b5, __global const float* b6, __global const float* b10, __global const float* b12, __global float* b3, __global float* b4, __global float* b7, __global float* b8, __global float* b9, __global float* b11, __global float* b13)
{
	...
	if(globalId < 32*8*8)
	{
		...
		if(process)
		{
			...
			data1_P = vload2(0,(__global float *)b8+poolInputOffset+16);
...
b11[globalId] = dotProduct;
	...
	if(globalId < 32)
	{
		for(int x=0; x<512/4; x++)
		{
			...
			wt.x= b12[32*(4*x)+globalId];
			wt.y= b12[32*((4*x)+1)+globalId];
			wt.z= b12[32*((4*x)+2)+globalId];
			wt.w= b12[32*((4*x)+3)+globalId];
			dotProduct += dot(wt,temp);
		}
