			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = floatC(0,(__global float *)b0+(j*32*32)+(inputRow_C+i)*32+inputCol_C);
					data0_C.x = 0;
				}
				...
				data0_C.w = 0.0;
				data1_C = floatC(0,(__global float *)b1+(plane_C*3*3*3)+(j*3*3)+i*3);
				data0_C.w = 0.0;
