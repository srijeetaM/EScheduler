__kernel void cnn_2_3_4_5_6_7_8_9( __global const float* b0, __global const float* b1, __global const float* b2, __global const float* b5, __global const float* b6, __global const float* b9, __global const float* b11, __global const float* b13, __global float* b3, __global float* b4, __global float* b7, __global float* b8, __global float* b10, __global float* b12, __global float* b14, __global float* b15)
{
				...
				for(int i=0;i<3;i++)
				{
					...
					else if(inputCol_C<0)
					{
						if(inputRow_C+i==0 && j==0)
						{
							...
							data0_C.xyz = vload3(0,(__global float *)b4+(j*8*8)+(inputRow_C+i)*8+inputCol_C);
						data0_C.x = 0;
					}
					else if(inputCol_C+3-1>=8)
					{
						...
						{
							data0_C.x = b4[(j*8*8)+(inputRow_C+i)*8+2];
							data0_C.y = b4[(j*8*8)+(inputRow_C+i)*8+3];
						}
						else
						data0_C.xyz = vload3(0,(__global float *)b4+(j*8*8)+(inputRow_C+i)*8+inputCol_C);
						data0_C.z = 0;
					...
					data0_C.w = 0.0;
					data1_C.xyz = vload3(0,(__global const float *)b5+(plane_C*3*3*32)+(j*3*3)+i*3);
					dots += dot(data0_C,data1_C);
				}
			}
	...
	if(globalId < 512)
	{
		for(int x=0; x<1024/4; x++)
		{
			...
			wt.x= b9[512*(4*x)+globalId];
			wt.y= b9[512*((4*x)+1)+globalId];
			wt.z= b9[512*((4*x)+2)+globalId];
			wt.w= b9[512*((4*x)+3)+globalId];
...
b10[globalId] = dotProduct;
	...
	if(globalId < 128)
	{
		for(int x=0; x<512/4; x++)
		{
			...
			wt.x= b11[128*(4*x)+globalId];
			wt.y= b11[128*((4*x)+1)+globalId];
			wt.z= b11[128*((4*x)+2)+globalId];
			wt.w= b11[128*((4*x)+3)+globalId];
			dotProduct += dot(wt,temp);
		}
b12[globalId] = dotProduct;
	...
	if(globalId < 16)
	{
		for(int x=0; x<128/4; x++)
		{
			...
			wt.x= b13[16*(4*x)+globalId];
			wt.y= b13[16*((4*x)+1)+globalId];
			wt.z= b13[16*((4*x)+2)+globalId];
			wt.w= b13[16*((4*x)+3)+globalId];
			dotProduct += dot(wt,temp);
		}
