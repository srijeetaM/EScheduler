...
class Kernel(object):
    def __init__(
        ...
        self.id = uid
        self.dag_id = dag_id
        self.num_input_buffers = num_input_buffers
        self.num_output_buffers = num_output_buffers
        self.num_variables = num_variables
        self.source_code = ""
        self.depth = depth
        self.input_buffer_names = []
        self.output_buffer_names = []
        self.buffer_info = {"input": [], "output": []}
        self.variable_info = []
        self.variable_names = []
    ...
    def dump_tinfo(self, name, source, dimension, filename,num_args=-1):
        ...
        outputBuffers = ""
        for output_buffer in self.buffer_info["output"]:
        ...
        outflow= ""
        outflow+= "data_outflow=" + str(int(self.name[-1:],10)+1) + ",0," + str(self.buffer_info["output"][-1][-2])
        # print outflow
        ...
        if int(self.name[-1:],10)+1 <nodesNum:
        	f.write(outflow)
        f.close()
