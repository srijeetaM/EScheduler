__kernel void cnn_1_2_3_4_5( __global const float* b0, __global const float* b2, __global const float* b3, __global const float* b6, __global const float* b8, __global float* b1, __global float* b4, __global float* b5, __global float* b7, __global float* b9)
{
	...
	if(globalId < 256)
	{
		for(int x=0; x<512/4; x++)
		{
			...
			wt.x= b6[256*(4*x)+globalId];
			wt.y= b6[256*((4*x)+1)+globalId];
			wt.z= b6[256*((4*x)+2)+globalId];
			wt.w= b6[256*((4*x)+3)+globalId];
