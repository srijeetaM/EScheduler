__kernel void cnn_2_3_4_5( __global const float* b0, __global const float* b1, __global const float* b2, __global const float* b5, __global const float* b7, __global float* b3, __global float* b4, __global float* b6, __global float* b8)
{
	...
	if(globalId < 256)
	{
		for(int x=0; x<512/4; x++)
		{
			...
			wt.x= b5[256*(4*x)+globalId];
			wt.y= b5[256*((4*x)+1)+globalId];
			wt.z= b5[256*((4*x)+2)+globalId];
			wt.w= b5[256*((4*x)+3)+globalId];
...
b6[globalId] = dotProduct;
	...
	if(globalId < 32)
	{
		for(int x=0; x<256/4; x++)
		{
			...
			wt.x= b7[32*(4*x)+globalId];
			wt.y= b7[32*((4*x)+1)+globalId];
			wt.z= b7[32*((4*x)+2)+globalId];
			wt.w= b7[32*((4*x)+3)+globalId];
			dotProduct += dot(wt,temp);
		}
