__kernel void cnn_0_1_2_3_4_5( __global const float* b0, __global const float* b1, __global const float* b2, __global const float* b5, __global const float* b6, __global const float* b9, __global const float* b11, __global float* b3, __global float* b4, __global float* b7, __global float* b8, __global float* b10, __global float* b12)
{
	...
	if(globalId < 256)
	{
		for(int x=0; x<512/4; x++)
		{
			...
			wt.x= b9[256*(4*x)+globalId];
			wt.y= b9[256*((4*x)+1)+globalId];
			wt.z= b9[256*((4*x)+2)+globalId];
			wt.w= b9[256*((4*x)+3)+globalId];
...
b10[globalId] = dotProduct;
	...
	if(globalId < 32)
	{
		for(int x=0; x<256/4; x++)
		{
			...
			wt.x= b11[32*(4*x)+globalId];
			wt.y= b11[32*((4*x)+1)+globalId];
			wt.z= b11[32*((4*x)+2)+globalId];
			wt.w= b11[32*((4*x)+3)+globalId];
			dotProduct += dot(wt,temp);
		}
