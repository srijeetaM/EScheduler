__kernel void cnn_1_2_3_4_5_6( __global const float* b0, __global const float* b2, __global const float* b3, __global const float* b7, __global const float* b9, __global float* b1, __global float* b4, __global float* b5, __global float* b6, __global float* b8, __global float* b10)
{
	...
	if(globalId < 32*8*8)
	{
		...
		inputImageOffset  = (n_P*32 + plane_P)*16*16;
		poolInputOffset = inputImageOffset + inputRow_P * 16 + inputCol_P;
		maxValue = b5[ poolInputOffset ];

		process = (inputRow_P + 1 < 16) && (inputCol_P + 1 < 16);

		if(process)
		{
			data0_P = vload2(0,(__global float *)b5+poolInputOffset);
			data1_P = vload2(0,(__global float *)b5+poolInputOffset+16);
	...
	if(globalId < 512)
	{
		for(int x=0; x<2048/4; x++)
		{
			...
			wt.x= b7[512*(4*x)+globalId];
			wt.y= b7[512*((4*x)+1)+globalId];
			wt.z= b7[512*((4*x)+2)+globalId];
			wt.w= b7[512*((4*x)+3)+globalId];
...
b8[globalId] = dotProduct;
	...
	if(globalId < 32)
	{
		for(int x=0; x<512/4; x++)
		{
			...
			wt.x= b9[32*(4*x)+globalId];
			wt.y= b9[32*((4*x)+1)+globalId];
			wt.z= b9[32*((4*x)+2)+globalId];
			wt.w= b9[32*((4*x)+3)+globalId];
			dotProduct += dot(wt,temp);
		}
