__kernel void cnn_3_4_5( __global const float* b0, __global const float* b2, __global const float* b4, __global float* b1, __global float* b3, __global float* b5)
{
	...
	if(globalId < 256)
	{
		for(int x=0; x<512/4; x++)
		{
			...
			wt.x= b2[256*(4*x)+globalId];
			wt.y= b2[256*((4*x)+1)+globalId];
			wt.z= b2[256*((4*x)+2)+globalId];
			wt.w= b2[256*((4*x)+3)+globalId];
...
b3[globalId] = dotProduct;
	...
	if(globalId < 32)
	{
		for(int x=0; x<256/4; x++)
		{
			...
			wt.x= b4[32*(4*x)+globalId];
			wt.y= b4[32*((4*x)+1)+globalId];
			wt.z= b4[32*((4*x)+2)+globalId];
			wt.w= b4[32*((4*x)+3)+globalId];
			dotProduct += dot(wt,temp);
		}
