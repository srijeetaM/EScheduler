__kernel void cnn_0_1_2_3( __global const float* b0, __global const float* b1, __global const float* b2, __global const float* b5, __global const float* b6, __global const float* b9, __global const float* b10, __global const float* b13, __global const float* b15, __global float* b3, __global float* b4, __global float* b7, __global float* b8, __global float* b11, __global float* b12, __global float* b14, __global float* b16)
{
...
/*			dotProduct += wt.x*temp.x + wt.y*temp.y + wt.z*temp.z + wt.w*temp.w;*/
	...
	if(globalId < 128)
	{
		for(int x=0; x<512/4; x++)
		{
			...
			wt= vload4(0,(__global const float *)b15+(globalId*512+4*x));
			dotProduct += dot(wt,temp);
/*			dotProduct += wt.x*temp.x + wt.y*temp.y + wt.z*temp.z + wt.w*temp.w;*/
		}
