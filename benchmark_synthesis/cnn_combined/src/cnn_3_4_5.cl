__kernel void cnn_3_4_5( __global const float* b0, __global const float* b1, __global const float* b3, __global const float* b5, __global float* b2, __global float* b4, __global float* b6, __global float* b7)
{
...
/*			dotProduct += wt.x*temp.x + wt.y*temp.y + wt.z*temp.z + wt.w*temp.w;*/
	...
	if(globalId < 16)
	{
		for(int x=0; x<128/4; x++)
		{
			...
			wt= vload4(0,(__global const float *)b5+(globalId*128+4*x));
			dotProduct += dot(wt,temp);
/*			dotProduct += wt.x*temp.x + wt.y*temp.y + wt.z*temp.z + wt.w*temp.w;*/
		}
